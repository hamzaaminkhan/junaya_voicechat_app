...
class _MomentsScreenState extends State<MomentsScreen> {
  ...
  final List<MomentItem> _moments = [
    ...
    const MomentItem(
      ...
      likes: 14,
      isLive: true,
      media: [MomentMedia.asset('assets/images/moments/ali_birthday.jpg')],
    ),
    const MomentItem(
      username: '♬ Zyni Malik 🌻💔',
  ...
  Future<void> _loadSavedMoments() async {
    try {
      final List<LocalMomentData> savedMoments = await _momentStorage
          .loadMoments();

      if (!mounted) return;

      setState(() {
        _moments.insertAll(0, savedMoments.map(_convertSavedMoment));
      });
    } catch (_) {
      if (!mounted) return;

      _showMomentMessage('Saved moments could not be loaded.', isError: true);
    }
  }
  ...
  Future<void> _openCreateMomentScreen() async {
    final CreatedMoment? createdMoment = await Navigator.push<CreatedMoment>(
      context,
    ...
    try {
      final LocalMomentData savedMoment = await _momentStorage.createMoment(
      ...
      _showMomentMessage('Moment could not be saved.', isError: true);
    }
  }
  ...
  Future<void> _editMoment(MomentItem moment) async {
          ...
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              shape: RoundedRectangleBorder(
                ...
                side: BorderSide(
                  color: const Color(0xFFA440F2).withValues(alpha: .55),
                ),
              ),
              ...
              title: const Row(
                children: [
                  Icon(Icons.edit_rounded, color: Color(0xFFFFD36A), size: 21),
                  SizedBox(width: 8),
                  Text(
                    'Edit Moment',
                    style: TextStyle(
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: const Color(0xFF9435CD).withValues(alpha: .7),
                      ),
                    ),
                    child: TextField(
    ...
    try {
      final LocalMomentData updatedMoment = await _momentStorage.updateMoment(
        moment: localMoment,
        text: updatedText,
      );
      ...
      _showMomentMessage('Moment could not be updated.', isError: true);
    }
  }
  ...
  Future<void> _deleteMoment(MomentItem moment) async {
          ...
          builder: (dialogContext) {
            return AlertDialog(
              ...
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(18),
                side: BorderSide(
                  color: const Color(0xFFFF4E80).withValues(alpha: .50),
                ),
              ),
              title: const Row(
                children: [
    ...
    try {
      ...
      setState(() {
        _moments.removeWhere((item) => item.localData?.id == localMoment.id);
      });
      ...
      _showMomentMessage('Moment could not be deleted.', isError: true);
    }
  }

  void _showMomentMessage(String message, {bool isError = false}) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: SafeArea(
        ...
        child: _CreateMomentButton(onTap: _openCreateMomentScreen),
      ),
      body: Stack(
        children: [
          const Positioned.fill(child: _MomentsBackground()),
          SafeArea(
            bottom: false,
            child: Column(
              children: [
  ...
  Widget _buildMomentsFeed() {
    return ListView.separated(
      ...
      padding: const EdgeInsets.fromLTRB(9, 4, 9, 145),
      ...
      itemBuilder: (context, index) {
        ...
        return MomentCard(
          key: ValueKey(moment.localData?.id ?? 'moment-sample-$index'),
          moment: moment,
          onEdit: moment.localData == null ? null : () => _editMoment(moment),
  ...
  Widget _buildFollowFeed() {
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return MomentCard(
          key: ValueKey(moment.localData?.id ?? 'follow-sample-$index'),
          moment: moment,
          onEdit: moment.localData == null ? null : () => _editMoment(moment),
          onDelete: moment.localData == null
              ? null
          ...
          child: Column(
            ...
            children: [
              Text(
                text,
                style: TextStyle(
                  color: selected ? const Color(0xFFFFD36A) : Colors.white54,
                  fontSize: 18,
...
class _MomentCardState extends State<MomentCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: const Color(0xFF8D36BD).withValues(alpha: .38),
          width: .8,
        ),
      ),
      child: Column(
  ...
  Widget _buildHeader(MomentItem moment) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        _ProfileAvatar(assetPath: moment.avatarAsset, isLive: moment.isLive),
        ...
        GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: const Color(0xFFFFC95C).withValues(alpha: .75),
              ),
            ),
            child: Icon(
  ...
  Widget _buildMedia(List<MomentMedia> media) {
    if (media.length == 1) {
      return LayoutBuilder(
        builder: (context, constraints) {
          final double mediaHeight = (constraints.maxWidth * .60).clamp(
            170.0,
            245.0,
          );
          ...
          return ClipRRect(
            ...
            child: SizedBox(
              ...
              height: mediaHeight,
              child: _MomentImage(media: media.first, fit: BoxFit.cover),
            ),
          );
        },
      );
    ...
    if (media.length == 2) {
      return LayoutBuilder(
        builder: (context, constraints) {
          final double mediaHeight = (constraints.maxWidth * .52).clamp(
            155.0,
            215.0,
          );
            ...
            child: Row(
              children: [
                Expanded(
                  child: ClipRRect(
                    borderRadius: BorderRadius.circular(11),
                    child: _MomentImage(media: media[0], fit: BoxFit.cover),
                  ),
                ),
                const SizedBox(width: 6),
                Expanded(
                  child: ClipRRect(
                    borderRadius: BorderRadius.circular(11),
                    child: _MomentImage(media: media[1], fit: BoxFit.cover),
                  ),
                ),
              ],
            ),
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        return ClipRRect(
          ...
          child: _MomentImage(media: media[index], fit: BoxFit.cover),
        );
      },
    );
  }
      ...
      children: [
        ...
        GestureDetector(
          ...
          child: Icon(
            ...
            color: _liked ? const Color(0xFFFF4E80) : const Color(0xFFFFD36A),
            size: 20,
          ),
        ),
        const SizedBox(width: 4),
  ...
  Widget _buildMomentOptions() {
    ...
    return PopupMenuButton<_MomentMenuAction>(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(13),
        side: BorderSide(color: const Color(0xFFA440F2).withValues(alpha: .45)),
      ),
      onSelected: (action) {
        switch (action) {
        ...
        return [
          if (widget.onEdit != null)
            const PopupMenuItem<_MomentMenuAction>(
              ...
              child: Row(
                children: [
                  Icon(Icons.edit_rounded, color: Color(0xFFFFD36A), size: 18),
                  SizedBox(width: 10),
                  Text(
                    'Edit moment',
            ...
            const PopupMenuItem<_MomentMenuAction>(
              ...
              child: Row(
                children: [
                  ...
                  Text(
                    'Delete moment',
                    style: TextStyle(color: Color(0xFFFF8BA8), fontSize: 13),
                  ),
                ],
              ),
            ),
...
class _ProfileAvatar extends StatelessWidget {
  ...
  const _ProfileAvatar({required this.assetPath, required this.isLive});
          ...
          Container(
            ...
            decoration: const BoxDecoration(
              ...
              gradient: LinearGradient(
                colors: [Color(0xFFFFD36A), Color(0xFFB635FF)],
              ),
            ),
            child: ClipOval(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: const LinearGradient(
          colors: [Color(0xFFF02B9B), Color(0xFFAF44F2)],
        ),
      ),
      child: Text(
        text,
...
class _MomentImage extends StatelessWidget {
  ...
  const _MomentImage({required this.media, this.fit = BoxFit.cover});
  ...
  Widget _fallback() {
    return Container(
      ...
      child: const Center(
        child: Icon(Icons.image_outlined, color: Color(0xFFB86AE3), size: 34),
      ),
    );
  }
}
        ...
        child: Ink(
          ...
          decoration: BoxDecoration(
            ...
            gradient: const LinearGradient(
              ...
              colors: [Color(0xFF8D27D0), Color(0xFF511278)],
            ),
            border: Border.all(
              color: const Color(0xFFFFD36A).withValues(alpha: .85),
              width: 1,
            ),
            boxShadow: [
              BoxShadow(
                color: const Color(0xFF7B24B5).withValues(alpha: .32),
                blurRadius: 10,
              ),
            ],
          ),
