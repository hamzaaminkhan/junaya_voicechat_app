...
class MediaStorage {
  ...
  Future<MomentMedia> saveFile({
    ...
    if(size > maxFileSize){
      throw Exception("Media exceeds limit");
    }

    final extension = _extension(source.path);

    if(extension == null || !_supported(extension)){
      throw Exception("Unsupported media");
    }

    final folder = await _directory();
    final id = _generateId();

    final filename = '${momentId}_${id}_$order$extension';
    final destination = p.join(folder.path, filename);
    final copied = await source.copy(destination);

    final mime = lookupMimeType(copied.path) ?? mimeType(extension);

    return MomentMedia(
      id: id,
      localPath: copied.path,
      remoteUrl: null,
      type: _mediaType(extension),
      order: order,
  ...
  Future<void> deleteMedia(List<MomentMedia> media) async {
    for(final item in media){
      await deleteSingle(item.localPath);

      if(item.thumbnail != null){
        await deleteSingle(item.thumbnail!);
      }
    }
  }

  Future<void> deleteSingle(String path) async {
    try{
  ...
  MomentMediaType _mediaType(String extension){
    switch(extension){
      case '.mp4':
      case '.mov':
      case '.avi':
  ...
  bool _supported(String extension){
    const formats = {
      '.jpg',
      '.jpeg',
      '.png',
  ...
  String? _extension(String path){
    final existing = p.extension(path).toLowerCase();
